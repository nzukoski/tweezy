...
function createTweetQuery(_term, _rpp, _pageNum, _maxTweets)
{
	_term = _term.replace("#","%23");
	_term = _term.replace(" ","%20");
...
function fetchNewTweets(searchParam)
{
    ...
    var url = createTweetQuery(searchParam, maxTweets, fetchNewTweets.pageNum);
    ajax_simple_get(url, "", false, function (_responseText)
		{
			var tempArray = jQuery.parseJSON(_responseText);
			if (tempArray != null)
			{
				for (index in tempArray)
				{
					tweetArray.push(tempArray[index].text);
				}
			}
		});
